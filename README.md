...
Edit `src/cli/commands/watch.ts`:
```typescript
debounceDelay: 2000,  // Change this
```
...
2. Trigger burst (in another terminal):
```bash
...
- [x] File watcher with burst detection
- [x] Auto-snapshot system
- [x] Local storage (~/.undoai)
- [x] CLI commands (watch, restore, stop, status)
- [x] Interactive restore

### 🔄 Phase 2: Smart Features (Next)
- [ ] Security scanning (detect secrets/API keys)
- [ ] Smart retention (keep last 24h, then hourly, then daily)
- [ ] Diff viewer (show changes before restore)
- [ ] Selective restore (choose specific files)

### 💎 Phase 3: Advanced (Future)
- [ ] VSCode extension
- [ ] Health check integration (auto-rollback on build fail)
- [ ] Export/import snapshots
- [ ] Team collaboration features (optional premium)

See [`fitur_premium.md`](./fitur_premium.md) for premium feature ideas.
...
Inspired by [mrq](https://getmrq.com) - we wanted a free, local alternative that anyone can use and modify.
